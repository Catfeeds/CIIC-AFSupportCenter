package com.ciicsh.gto.afsupportcenter.util.poi;

import com.ciicsh.gto.afsupportcenter.util.poi.exception.CellGetOrSetException;
import com.ciicsh.gto.afsupportcenter.util.poi.exception.CellRuleException;
import com.ciicsh.gto.afsupportcenter.util.poi.exception.XSSFCellTypeException;
import com.ciicsh.gto.afsupportcenter.util.poi.model.*;
import com.ciicsh.gto.afsupportcenter.util.poi.utils.BeanUtils;
import com.ciicsh.gto.afsupportcenter.util.poi.utils.CellDataConverter;
import com.ciicsh.gto.afsupportcenter.util.poi.utils.DigestUtils;
import com.ciicsh.gto.afsupportcenter.util.poi.utils.FieldUtils;
...
//        createHideSelectSheet(result.getResultWorkbook(),sheetSettingsList);
    ...
    private List<CellSettings> getAllCell(List<CellSettings> cellSettingsList){
        LinkedList<CellSettings> cellArrays = new LinkedList<CellSettings>();
        cellArrays.addAll(
    ...
    private Object getCellValue(OfficeIoResult result,
        ...
        if (cellSettings.getSelect()){
            ...
            if (!cellSettings.getSelectCascadeFlag()) {
                formulaString = sheetSettings.getSheetSeq() + "_" + cellSettings.getKey() + "_TEXT";
                mapList = (List<String>) result.getSelectMap().get(DigestUtils.digestFormulaName(formulaString));
            }else {
                formulaString = sheetSettings.getSheetSeq() + "_" + cellSettings.getKey() + "_" + selectTargetValueMap.get(cellSettings.getSelectTargetKey()) + "_TEXT";
                mapList = (List<String>)result.getSelectMap().get(DigestUtils.digestFormulaName(formulaString));
            }
            if (mapList != null){
    ...
    private void setCellDataValue(Sheet sheet, Cell cell, SheetSettings sheetSettings, CellSettings cellSettings, Object dataBean) throws CellGetOrSetException {
        ...
        if (cellSettings.getSelect()) {
            ...
            if (cellSettings.getSelectCascadeFlag()){
                ...
                formulaString.append("),selectionText");
    ...
    private void createHideSelectSheet(Workbook workbook, List<SheetSettings> sheetSettingsList) {
        Sheet selectTextSheet = workbook.createSheet("selectionText");
        Sheet selectValueSheet = workbook.createSheet("selectionValue");
        ...
        sheetSettingsList.forEach(sheetSettings -> {
            List<CellSettings> cellArrays = getAllCell(sheetSettings.getCellSettingsList());
            // 先处理没有联动的下拉
            cellArrays.stream()
                        ...
                        if (cellSettings.getSelectSourceList() != null && cellSettings.getSelectSourceList().size() > 0){
                            ...
                            if (targetTextArray != null && targetTextArray.length > 0){
                                if (targetValueArray != null && targetValueArray.length > 0) {
                                    ...
                                    LinkedMap addTextSelectMap = new LinkedMap();
                                    LinkedMap addValueSelectMap = new LinkedMap();
                                    ...
                                    cellSettings.getSelectSourceList().forEach(obj -> {
                                        int matchIndex = -1;
                                        try {
                                            matchIndex = ArrayUtils.indexOf(targetValueArray, String.valueOf(BeanUtils.invokeGetter(obj, cellSettings.getBingKey())));
                                            if (matchIndex >= 0) {
                                                ((List)addTextSelectMap.get(targetTextArray[matchIndex])).add(String.valueOf(BeanUtils.invokeGetter(obj, cellSettings.getCellSelectSettings().getName())));
                                                ((List)addValueSelectMap.get(targetValueArray[matchIndex])).add(String.valueOf(BeanUtils.invokeGetter(obj, cellSettings.getCellSelectSettings().getKey())));
                                            }
                                        } catch (CellGetOrSetException e) {
                                            log.warn(e.getMessage());
                                        }
                                    ...
                                    IntStream.range(0,addTextSelectMap.size())
                                            .forEach(index ->{
                                                String[] addTextArray = new String[((List)addTextSelectMap.getValue(index)).size()];
                                                ((List)addTextSelectMap.getValue(index)).toArray(addTextArray);
                                                String[] addValueArray = new String[((List)addValueSelectMap.getValue(index)).size()];
                                                ((List)addValueSelectMap.getValue(index)).toArray(addValueArray);

                                                String formulaStr = DigestUtils.digestFormulaName(sheetSettings.getSheetSeq() + "_" + cellSettings.getKey() + "_" + addTextArray[0] + "_TEXT");
                                                addTextArray[0] = sheetSettings.getSheetSeq() + "_" + cellSettings.getKey() + "_" + addTextArray[0];
                                                addTextArray = ArrayUtils.insert(1,addTextArray,formulaStr);
                                                addValueArray[0] = sheetSettings.getSheetSeq() + "_" + cellSettings.getKey() + "_" + addValueArray[0];
                                                addValueArray = ArrayUtils.insert(1,addValueArray,formulaStr);

                                                setSelectRow(selectTextSheet, selectValueSheet, args[0], addValueArray, addTextArray,true);
                                                createSelectNameList(selectTextSheet.getSheetName(), workbook, formulaStr, args[0]++, addTextArray.length, cellSettings.getSelectCascadeFlag());
                                            });
                                }
                            }
                        }
        ...
        workbook.setSheetHidden(workbook.getSheetIndex("selectionValue"), true);
        workbook.setSheetHidden(workbook.getSheetIndex("selectionText"), true);
    }
    ...
    private void getSelectSheetMap(OfficeIoResult officeIoResult){
        ...
        list.stream().forEach(name -> {
            ...
            Sheet textSheet = officeIoResult.getResultWorkbook().getSheet("selectionText");
            Sheet valueSheet = officeIoResult.getResultWorkbook().getSheet("selectionValue");
