...
public class TaskServiceImpl extends ServiceImpl<TaskMapper, Task> implements TaskService {
    ...
    public Boolean deleteTaskById(String taskId) {
        ...
        List<TimedTask> list = timedTaskService.selectList(new EntityWrapper<TimedTask>().eq("task_id", taskId));
        if (!list.isEmpty()) {
            timedTaskService.updateTimedTaskByTaskId(taskId);
        }
        return true;
    }
    ...
    private void insertTimeTask(TaskDetialDTO taskDetialDTO) {
        ...
        timedTask.setTemplateType(taskDetialDTO.getTemplateType());
