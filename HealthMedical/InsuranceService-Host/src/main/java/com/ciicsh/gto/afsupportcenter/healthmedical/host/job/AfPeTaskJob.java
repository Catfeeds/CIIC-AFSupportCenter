...
public class AfPeTaskJob {
    ...
    public JsonResult findBespeakPeId() {
        ...
        afPeTaskList.stream().forEach((AfPeTask i) -> {
            String url = "http://10.17.3.1:9999/FileOperationForJSON.svc/GetPhysicalExaminationInformation/610103197212302474?idNum="+i.getIdNum();
            ...
            afPeTask.setIdNum(jo.getString("idNum"));
    ...
    public JsonResult updateAfPeTaskByBespeakPeId() {
        ...
        //todo 调方法获取sign
        afPeTaskList.stream().forEach((AfPeTask i) -> {
            String url = "http://51joying.iyaoshow.com/api/physical/GetTijianEmployeeID?ComNo="+i.getComNo()+"&TiJianEmployeeID="
                +i.getBespeakPeId()+"&_time="+System.currentTimeMillis()+"&_sign=";
            JSONObject jo = restTemplate.getForObject(url, JSONObject.class);
            Object obj = jo.getJSONObject("data");
